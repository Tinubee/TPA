...
namespace TPA.Schemas
{
    public enum 큐알코드구분
    {
        ...
        Supplier,
        [Description("Setllantis Part No.")]
        Setllantis,
        [Description("Traceability Code (CSC)")]
        CscCode,
        [Description("CSC Mac Address")]
        CscMac,
        [Description("Others")]
        Others,
        [Description("CSC Lot No.")]
        CscLot,
        [Description("HW Number")]
        HwNumber,
        [Description("SW Number")]
        SwNumber,
    }
    ...
    public class 큐알검증정보
    {
        ...
        public String 샘플코드()
        {
            ...
            Match match = Regex.Match(표현형식, 랜덤패턴);
        ...
        private static String 랜덤패턴 = $@"\[a-z0-9\]{길이패턴}";
    ...
    public class 큐알검증
    {
        ...
        public Boolean 중복검사(String 큐알코드, Int32[] indexs, out String 오류내용)
        {
            ...
            foreach (Int32 idx in results.Keys) {
                ...
                중복오류.Add(정보.코드구분.ToString() + "중복");
            }
            if (중복오류.Count > 0) {
                오류내용 = String.Join(",", 중복오류.ToArray());
                return false;
            }
            return true;
        }
