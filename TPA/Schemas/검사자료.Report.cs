...
namespace TPA.Schemas.Report
{
    public class 검사내역 : IDisposable
    {
        ...
        public void Init(모델구분 모델)
        {
            ...
            col.ExtendedProperties[Band] = Summary;
            ...
            col.Caption = "Result";
            col.ExtendedProperties[FormatType] = DevExpress.XtraGrid.Columns.FixedStyle.Left;
            col.ExtendedProperties[Band] = Summary;
            ...
            foreach (검사정보 설정 in Global.모델자료.GetItem(모델구분).검사설정)
            {
                ...
                col.Caption = 설정.검사항목.ToString();
                col.ExtendedProperties[Band] = Utils.GetDescription(설정.검사장치);
                col.ExtendedProperties[FormatType] = DevExpress.Utils.FormatType.Numeric;
                if (설정.측정단위 == 단위구분.mm)
                {
        ...
        {
            if (col.ExtendedProperties.Contains(Band))
                return Convert.ToString(col.ExtendedProperties[Band]);
            return String.Empty;
        }

        public Boolean GetVisible(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(Visible))
                return Convert.ToBoolean(col.ExtendedProperties[Visible]);
            return true;
        }

        public DevExpress.Utils.FormatType GetFormatType(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(FormatType))
                return (DevExpress.Utils.FormatType)col.ExtendedProperties[FormatType];
            return DevExpress.Utils.FormatType.None;
        }

        public String GetFormatString(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(FormatString))
                return Convert.ToString(col.ExtendedProperties[FormatString]);
            return String.Empty;
        }

        public Object GetStandardValue(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(Standard))
                return col.ExtendedProperties[Standard];
            return null;
        }

        public FixedStyle GetFixedStyle(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(FixedStyle))
                return (FixedStyle)col.ExtendedProperties[FixedStyle];
            return DevExpress.XtraGrid.Columns.FixedStyle.None;
        }

        public String GetToolTip(DataColumn col)
        {
            if (col.ExtendedProperties.Contains(ToolTip))
                return Convert.ToString(col.ExtendedProperties[ToolTip]);
            return String.Empty;
        }
