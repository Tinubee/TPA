...
namespace TPA.UI.Controls
{
    public partial class SetInspection : XtraUserControl
    {
        ...
        private void DeleteClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (!Utils.Confirm(번역.삭제확인)) return;
            검사.Remove(설정);
        }
        ...
        private void 설정저장(object sender, EventArgs e)
        {
            검사설정자료 설정 = this.검사설정;
            if (설정 == null) return;
            if (!Utils.Confirm(번역.저장확인)) return;
            if (설정.Save()) Global.정보로그(검사설정자료.로그영역.GetString(), 번역.설정저장, 번역.저장완료, true);
        }

        private void 마스터보정(object sender, EventArgs e)
        {
            Forms.MasterCalibForm MasterCalibForm = new Forms.MasterCalibForm();
            MasterCalibForm.StartPosition = System.Windows.Forms.FormStartPosition.CenterParent;
            MasterCalibForm.Init();
            MasterCalibForm.ShowDialog();
        }
